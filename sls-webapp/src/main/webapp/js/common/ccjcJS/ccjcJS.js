...
var JCRegExp = {
    ...
    /**
     * @static isbn：10位数字 或者用-分为4组
     * @static       13位数字 或者用-或空格分为4组。
     */
    isbnFormat: {
        code: /^\d+$|^\d+(\-\d+){2}\-\d$|^\d+((\-|\s)\d+){3}(\-|\s)\d$/,
        msg: "请输入正确的ISBN码！<br/>10位ISBN：由10位数字组成，若用\"-\"分组，请分为4组。<br/>13位ISBN：由13为数字组成，若用\"-\"或\"空格\"分组，请分为5组"
    },
    isbnLength: {
        code: /^\d{10}$|^[0-9\-\s]{13}$|^[0-9\-\s]{17}$/,
        msg: "请输入正确的ISBN码！<br/>ISBN码应为10位或13位数字，若使用分隔符，则应为13位或17位"
    },
    /**
     * @static 数字、字母、下划线
     */
    ...
    intDeal: function (e) {
        ...
        if (eType == "keydown") {
            ...
            if (key == 40 && this.value.isNotEmpty().bool && (min == undefined || (min != undefined && parseInt(this.value, 10) - range >= parseInt(min, 10)))) {
                if ($(this).attr("positive") == "positive" && parseInt(this.value, 10) - range < "0") {
                    return true
                }
                this.value = parseInt(this.value, 10) - range;
                return true;
    ...
    floatDeal: function (e) {
        ...
        if (eType == "keydown") {
            ...
            if ((key <= 57 && key >= 48) || (key >= 96 && key <= 105)) {
                ...
                return (max == undefined || this.value != max || JCTools.getSelectText(this).length > 0) && (precision == undefined || this.value.split(".").length < 2 || (this.value.split(".")[1]).length != parseInt(precision, 10) || JCTools.getPositionForInput(this) < this.value.indexOf(".") + 1 || JCTools.getSelectText(this).length > 0)
            }
            if (key == 110 || key == 190) {
...
//    },
    ...
    dateDeal: function () {
        ...
        WdatePicker && WdatePicker({skin: 'whyGreen', dateFmt: iType});
    },
...
//    } ,
    ...
    enterToTab: function (inputs) {
        if (inputs == undefined) {
            inputs = $("input:visible,select").not("input:disabled").not("input[readonly]");
        }
...
var JCCheckFunc = {
    ...
    /**
     * @description 身份证号码合理性验证
     * @param {String} objId 要设置为true的结点id
     * @return {Object} returnObj returnObj={bool：bool类型值，obj：jquery对象}
     * @example checkIdCardNumLegality:["objId"]
     */
    checkIdCardNumLegality: function () {
        arguments = arguments[0];
        var objId = arguments[0];
        var obj = $("#" + objId);
        var returnObj = obj.val().isChinaIDCard();
        returnObj.errMsg = "请输入正确的身份证号码！";
        returnObj.obj = obj;
        return returnObj;
    },
    /**
    ...
    checkFrameEmpty: function () {
        arguments = arguments[0];
        var objText = arguments[1];
        var frameObjId = arguments[0];
        var objId = arguments[2];
        var obj = $("#" + objId);
        var frameObj = $('#' + frameObjId);
        var returnObj = frameObj.val().isNotEmpty();
        returnObj.obj = obj;
        if (!returnObj.bool) {
            returnObj.errMsg = objText == undefined || objText == "" ? "请选择正确的内容！" : objText + "请选择正确的内容！";
        }
        return returnObj;
    },
    /**
    ...
    /**
     * @description 进行dwr校验
     * @param {Function} dwrFuncName 自定义的dwr校验函数名，此函数须有返回值returnObj，并包括bool,obj,errMsg字段
     * @return {Object} returnObj returnObj={bool：bool类型值，obj：显示错误信息的jquery对象，errMsg：错误信息字符串}
     * @example dwr:[dwrtest]
     * function dwrtest(){
     *     var returnObj={}
     *     SalebillFalseController.saveSaleBill("1","jim啊啊",{saleId:'1',saleType:$('#clientName').val()},function(data){
     *         returnObj.obj=$("#clientName")
     *         returnObj.errMsg="aaa"
     *         returnObj.bool=false;
     *     });
        ...
        var ccjc = (function () {
            ...
            ccjc.fn = ccjc.prototype = {
                ...
                methods: {
                    ...
                    checkIdCardNumLegality: JCCheckFunc.checkIdCardNumLegality,
                    checkNum: JCCheckFunc.checkNum,
                    checkEmpty: JCCheckFunc.checkEmpty,
                    checkFrameEmpty: JCCheckFunc.checkFrameEmpty,
...
//                        JCTools.firstFocus();
                        this.enterToTabObj = JCTools.enterToTab($("input:visible,select,a.search_button").not("input:disabled").not("input[readonly]"));
                        JCTools.forbidBack();
                    }
                    return this;
                },
